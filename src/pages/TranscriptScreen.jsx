...
export default function TranscriptScreen() {
    ...
    const load = async () => {
      try {
        ...
        try {
          ...
          logError('Failed to load quiz attempts (local fallback parse)', {
            error: e && e.message ? e.message : e,
          });
        }
        logError('Failed to load quiz attempts from backend', {
  ...
  const assignmentAverage =
    graded && graded.length
      ? Math.round(
          graded.reduce((acc, a) => acc + (parseFloat(a.score || 0) || 0), 0) / graded.length
        )
      : 0;
  ...
  return (
    ...
    <div className="transcript__container">
      ...
      <div className="transcript__stats-grid">
        <StatCard
          icon={<BadgeCheck size={150} />}
          percentage={Math.round(
            (assignmentAverage + quizAverage) / ((assignmentAverage > 0) + (quizAverage > 0) || 1)
          )}
        ...
        />
        <StatCard
          icon={<NotepadText size={150} />}
          percentage={assignmentAverage}
          label="Assignment Average"
          sub={`${graded.length} assignments graded`}
          borderColor="#60a5fa"
        />
        <StatCard
          ...
          label="Quiz Average"
          sub={`${quizAttempts.length} quizzes taken`}
          borderColor="#22d3ee"
        />
      </div>
...
const StatCard = ({ percentage, label, sub, icon }) => (
  <div className="transcript__stat-card">
    ...
    <div className="transcript__stat-icon">{icon}</div>
  </div>
);
...
const ListRow = ({ icon, title, score }) => (
  <div className="transcript__milestones-list">
    ...
    <h3 className="score-main" style={{ color: '#22d3ee' }}>
      {score}%
    </h3>
  </div>
);
