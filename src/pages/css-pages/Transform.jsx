    ...
    <section className="course-content">
      ...
      <section>
        <div className="course-content__container">
          <p>
            The <code className="code">transform</code> property in CSS allows you to visually
            manipulate an element’s shape, size, position, and rotation without affecting the
            document flow. This is widely used in animations, UI interactions, and responsive design
            effects.
          </p>
...
transform: translateY(-50px);`}
          ...
          <ExampleCode language="css">{`transform: scaleX(2);`}</ExampleCode>

          <h3>skew(x-angle, y-angle)</h3>
          <p>Skews the element along X and/or Y axes.</p>
          <ExampleCode language="css">{`transform: skew(30deg, 10deg);`}</ExampleCode>

          <h4>skewX(value) / skewY(value)</h4>
          <p>Skew only one axis.</p>
          <ExampleCode language="css">{`transform: skewX(25deg);`}</ExampleCode>
          ...
          <p>
            <strong>Order matters</strong> – transforms are applied in sequence.
          </p>
   ...
   transform: skewX(20deg);
          ...
          <VideoBox title="Transform" code="0BXG-fMTWF4" />
        </div>
      </section>

      <QuizBox currentCategory="css2" currentTopic="Transform" />
      <PrevNextBtn prevPath="/css_opacity" nextPath="/css_transition" />
    </section>
  );
}
