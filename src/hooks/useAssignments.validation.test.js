...
describe('useAssignments validation and admin flows', () => {
  ...
  describe('Validation - submitAssignment', () => {
    ...
    it('rejects submission with notes exceeding 500 characters', async () => {
      ...
      const invalidPayload = { submissionUrl: 'https://example.com/submission', notes: longNotes };
      await act(async () => {
        try {
          await result.current.submitAssignment('1', invalidPayload);
        } catch (err) {
    ...
    it('accepts valid submission with URL and notes', async () => {
      const { result } = renderHook(() => useAssignments());
      const validPayload = { submissionUrl: 'https://example.com/submission', notes: 'Here is my completed assignment' };
      let response;
      await act(async () => {
  ...
  describe('Validation - gradeAssignment', () => {
    ...
    it('accepts valid grade with score and feedback', async () => {
      const { result } = renderHook(() => useAssignments(TOKEN));
      server.use(
        http.put('*/api/assignments/1/grade', () => HttpResponse.json({ success: true })),
    ...
    it('succeeds grading but sets error when refresh fails', async () => {
      ...
      server.use(
        http.put('*/api/assignments/1/grade', () => HttpResponse.json({ success: true })),
        http.get('*/api/assignments/admin/graded', () => new HttpResponse(null, { status: 500 }))
      );
      const gradeResult = await act(async () => {
  ...
  describe('Validation - createAssignment', () => {
    ...
    it('accepts valid assignment creation', async () => {
      const { result } = renderHook(() => useAssignments(TOKEN));
      server.use(
        http.post('*/api/assignments', () => HttpResponse.json({ success: true }, { status: 201 })),
    ...
    it('surfaces a server error when the create request fails', async () => {
      const { result } = renderHook(() => useAssignments(TOKEN));
      server.use(
        http.post('*/api/assignments', () => HttpResponse.json({ message: 'Course is archived' }, { status: 400 }))
      );
      const createResult = await act(async () => {
        return await result.current.createAssignment('Valid Title', 'Description', '2026-12-31', 'react');
      });
  ...
  describe('Validation - updateAssignment', () => {
    ...
    it('accepts a valid assignment update and refreshes the list', async () => {
      const { result } = renderHook(() => useAssignments(TOKEN));
      server.use(
        http.put('*/api/assignments/1', () => HttpResponse.json({ success: true })),
        http.get('*/api/assignments/admin/all', () => HttpResponse.json({ assignments: [{ id: '1', title: 'Updated title', courseId: 'html' }] }))
      );
      const updateResult = await act(async () => {
    ...
    it('surfaces a server error when the update request fails', async () => {
      const { result } = renderHook(() => useAssignments(TOKEN));
      server.use(
        http.put('*/api/assignments/1', () => HttpResponse.json({ message: 'Assignment locked' }, { status: 409 }))
      );
      const updateResult = await act(async () => {
        return await result.current.updateAssignment('1', 'Updated title', 'Description', '2026-12-31', 'html');
      });
  ...
  describe('deleteAssignment', () => {
    ...
    it('deletes an assignment and removes it from allAssignments', async () => {
      server.use(
        http.get('*/api/assignments/admin/all', () => HttpResponse.json({ assignments: [{ id: '1', title: 'To be deleted', courseId: 'html' }, { id: '2', title: 'Stays around', courseId: 'react' }] })),
    ...
    it('surfaces a server error when the delete request fails and keeps state unchanged', async () => {
      server.use(
        http.get('*/api/assignments/admin/all', () => HttpResponse.json({ assignments: [{ id: '1', title: 'Protected', courseId: 'html' }] })),
        http.delete('*/api/assignments/1', () => HttpResponse.json({ message: 'Cannot delete a graded assignment' }, { status: 403 }))
      );
      const { result } = renderHook(() => useAssignments(TOKEN));
      await act(async () => {
        await result.current.fetchAllAssignments();
  ...
  describe('gradeAssignment - server error path', () => {
    ...
    it('surfaces a server error when the grade request fails', async () => {
      server.use(
        http.put('*/api/assignments/1/grade', () => HttpResponse.json({ message: 'Grading window closed' }, { status: 400 }))
      );
      const { result } = renderHook(() => useAssignments(TOKEN));
      const gradeResult = await act(async () => {
